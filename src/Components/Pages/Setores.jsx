...
export default function Setores() {
  ...
  const saveRegister = () => {
    ...
    if (register.name.trim()) {
      ...
      const existingSetor = setores.find(
        (setor) => setor.name === register.name,
      );
      if (existingSetor) {
        toast.current.show({
          severity: "error",
      ...
      api
        ...
        .then((response) => {
          ...
          toast.current.show({
            severity: "success",
            summary: "Successful",
            detail: "Registro de Setor Salvo com Sucesso",
            life: 3000,
          });
          fetchSetores();
        })
      ...
      <div className={styles.cadastro}>
        <div className="datatable-crud-demo">
          ...
          <Dialog
            ...
            style={{ width: "450px" }}
            header="Cadastro de Setores"
            modal
            className="p-fluid"
            footer={register.id ? registerDialogFooter2 : registerDialogFooter}
            onHide={hideDialog}
          >
            {register.image && (
              <img
            ...
            <div className="field">
              ...
              {submitted && !register.name && (
                <small className="p-error">Nome necessário.</small>
              )}
            </div>
            ...
            <div className="field">
              ...
              <InputText
                id="code"
                value={register.code ? register.code : ""}
                onChange={(e) => onInputChange(e, "code")}
                required
                className={classNames({
                  "p-invalid": submitted && !register.code,
                })}
              />
              {submitted && !register.code && (
                <small className="p-error">Abreviação necessária.</small>
              )}
            </div>
          </Dialog>
